...
cattle_n_geo <- na.omit(livestock[,c("cattle_dry_lon","cattle_dry_lat","n_livestock")])%>%
  setNames(c("longitude","latitude","n_livestock"))

pig_n_geo <- na.omit(livestock[,c("pig_dry_lon","pig_dry_lat","n_livestock")])%>%
  setNames(c("longitude","latitude","n_livestock"))

liv_geo <- rbind(cattle_n_geo,pig_n_geo,deparse.level = 0)%>%
  as.data.frame()%>%
...
pig_n_geo <- na.omit(df_pig[,c("pig_dry_lon","pig_dry_lat","n_pigs")])%>%
...
df_cow <- mini_cen_main %>% filter_at(vars(hh_n_cows_less_than_1_year,hh_n_cows_greater_than_1_year),all_vars(!is.na(.)))%>%
  ...
  separate(hh_animals_distance_cattle_rainy_season_geo, c("cattle_rain_lat","cattle_rain_lon","east","north")," ")%>%
  select(-c("east","north"))%>%
  select(c("cattle_dry_lon","cattle_dry_lat","n_cows"))%>%
  na.omit()%>%
...
```{r}
pnts <- st_intersection(df_cow,cost_acd_buffer)%>%
  st_transform(4326)
```
...
boh_liv_df %>%
  mutate(QL = if_else(n_livestock <= 2,1,
                      if_else(n_livestock <= 5 & n_livestock > 2,2,
                              if_else(n_livestock <= 8 & n_livestock > 5, 3,4))),
         DL = if_else(distance <= 165.25,1,
                      if_else(distance <= 330.5 & distance > 165.25,2,
                              if_else(distance <= 495.75 & distance> 330.5,3,4))),
         RiskL = QL/DL)%>%
...
boh_pig_df %>%
  mutate(QP = if_else(n_pigs <= 2,1,
                      if_else(n_pigs <= 5 & n_pigs > 2,2,
                              if_else(n_pigs <= 8 & n_pigs > 5, 3,4))),
         DP = if_else(distance <= 165.25,1,
                      if_else(distance <= 330.5 & distance > 165.25,2,
                              if_else(distance <= 495.75 & distance> 330.5,3,4))),
         RiskP = QP/DP)%>%
...
write.csv(final, "COST_ACD_variables.csv")
```
