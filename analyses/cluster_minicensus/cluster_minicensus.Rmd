...
get_eligible_for_ttm <- function(out, p_pregnant = 0.07, p_no_participate = 0.05){
  ...
  next_row <- tibble(assignment_group = c('Total estimated eligibles'),
                     ...
                     `Non-children` = sum(no_buffer$`Non-children`),
                     ...
                     `Estimated non-participation` = sum(no_buffer$`Estimated non-participation`))
  final_row <- next_row
  ...
  final <- list()
  final[[1]] <- pdx %>%
        mutate(Final = `Non-children` - `Estimated pregnant` - `Estimated non-participation`) %>%
...
cat(paste0('Estimated ', scales::comma(f[[2]]), ' people to be treated with ivermectin.'))
```
