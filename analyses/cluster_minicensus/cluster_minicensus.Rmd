...
# Define function for carlos table
carlos_table <- function(out, country = 'Tanzania'){
  ...
  if(country == 'Mozambique'){
    ...
    mutate(eligibles = n_humans - n_children) %>%
    mutate(non_child_females = n_females - n_girls) %>%
    group_by(`Assignment group` = assignment_group) %>%
    summarise(Households = sum(n_households),
              Humans = sum(n_humans),
              Children = sum(n_children),
             `Adults` = sum(eligibles),
             `Women of reproductive age` = sum(n_reproductive),
             `Cows eligible for TTM`= sum(cows_1_year_plus),
             `Pigs eligible for TTM` = sum(pigs_6_weeks_plus)
             )
  } else {
  pd <- out$hamlet_xdf %>%
    mutate(eligibles = n_humans - n_children) %>%
    ...
    summarise(Households = sum(n_households),
              Humans = sum(n_humans),
              `Non-eligible children` = sum(under5s),
              `Eligible children` = sum(n_children),
             `Adults` = sum(eligibles),
             `Women of reproductive age` = sum(n_reproductive),
             `Cows eligible for TTM`= sum(cows_1_year_plus),
             `Pigs eligible for TTM` = sum(pigs_6_weeks_plus)
...
out <- try_clusters_hh_level(the_country = 'Mozambique',
                         ...
                         df = df[keep_index,],
                         locations_list = locations_list[keep_index])
knitr::kable(carlos_table(out = out, country = 'Mozambique'))
