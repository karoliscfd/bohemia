...
if('pre_load.RData' %in% dir()){
  ...
  difficulty <- read_csv('Bohemia hamlets_Accessibility.csv')
  difficulty <- difficulty %>% dplyr::select(code = hamlet_code, difficulty = Accessibility)
  ...
  library(readxl)
  sizes_df <- read_excel('Children cluster size etcTZA.xlsx', skip = 1)
  sizes_df <- sizes_df[,c(1,6)]
  names(sizes_df) <- c('n_children', 'n_clusters')
  sizes_df <- sizes_df %>% filter(!is.na(n_clusters))
...
if('master.RData' %in% dir()){
  ...
  buffer_distances <- buffer_distances <- c(400)  #c(100, 200, 300, 400, 500, 600)
  n_childrens <- c(20)   #c(6,9,11)#  c(30, 35)#c(8, 13, 18) #c(7, 12, 17, 22, 27)#c(5, 10, 15, 20, 25)
