...
REPORTS
We should be able to generate the below tables and summary reports:
...
tbl3<-tbl3_data %>% group_by(hf_name) %>% summarize(one = sum(one), ape_count=sum(ape_count), phase1=sum(phase1), result=sum(result), phase2=sum(phase2), .groups = 'drop') %>% as.data.frame() #  "drop", "keep", "rowwise"
knitr::kable(head(tbl3), col.names = c("Health facility", "No. PMSS questionnaire from Health Facility", "No. PMSS questionnaire from APEs", "No. Malaria register completion Baseline Reports (Phase 1)", "No. of intervention Phase 2 forms required", "No. Malaria register completion Post-Baseline Reports (Phase 2)"),  "pipe")
...
library("ggplot2")
#ggplot(data=tbl2_data, mapping = aes(x=hf_name)) +  geom_histogram(stat="count")
ggplot(tbl1, aes(Health_facility,one, label = Health_facility)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~Month, scales="free") +
  labs(y="Number of cases", x = "No. PMSS questionnaire from Health Facility")

ggplot(tbl1, aes(Health_facility,ape_count, label = Health_facility)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~Month, scales="free") +
  labs(y="Number of cases", x = "No. PMSS questionnaire from APEs")

ggplot(tbl2, aes(hf_name, phase1, label = hf_name)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~start_time, scales="free") +
  labs(y="Number of cases", x = "No. Malaria register completion Baseline Reports (Phase 1)")

ggplot(tbl2, aes(hf_name, result, label = hf_name)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~start_time, scales="free") +
  labs(y="Number of cases", x = "No. of intervention Phase 2 forms required")

ggplot(tbl2, aes(hf_name, phase2, label = hf_name)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~start_time, scales="free") +
  labs(y="Number of cases", x = "No. Malaria register completion Post-Baseline Reports (Phase 2)")
