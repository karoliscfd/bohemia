window.odkCommonDefinitions = {
  "_tokens": {
    "op_title": {
      "string_token": "op_title",
      "text": {
        "default": "BOHEMIA CENSUS",
        "pt": "Censo do BOHEMIA"
      },
      "_row_num": 2
    },
    "fw_id_title": {
      "string_token": "fw_id_title",
      "text": {
        "default": "Fieldworker ID",
        "pt": "ID do Inquiridor"
      },
      "_row_num": 3
    },
    "hh_id_placeholder": {
      "string_token": "hh_id_placeholder",
      "text": {
        "default": "ABC-123",
        "pt": "ABC-123",
        "sw": "ABC-123"
      },
      "_row_num": 4
    },
    "member_placeholder": {
      "string_token": "member_placeholder",
      "text": {
        "default": "Enter Name",
        "pt": "Insira o Nome"
      },
      "_row_num": 5
    },
    "fw_id_label": {
      "string_token": "fw_id_label",
      "text": {
        "default": "Fieldworker ID:",
        "pt": "ID do Inquiridor"
      },
      "_row_num": 6
    },
    "fw_id_button": {
      "string_token": "fw_id_button",
      "text": {
        "default": "Enter Fieldworker ID",
        "pt": "Insira o ID do Inquiridor"
      },
      "_row_num": 7
    },
    "new_hh_button": {
      "string_token": "new_hh_button",
      "text": {
        "default": "Add New Household",
        "pt": "Adicionar novo Agregado"
      },
      "_row_num": 8
    },
    "select_hh_button": {
      "string_token": "select_hh_button",
      "text": "Select Household",
      "_row_num": 9
    },
    "send_button": {
      "string_token": "send_button",
      "text": {
        "default": "Send Data",
        "pt": "Enviar Dados"
      },
      "_row_num": 10
    },
    "scan_button": {
      "string_token": "scan_button",
      "text": {
        "default": "Scan QR Code",
        "pt": "Scanear Codigo QR"
      },
      "_row_num": 11
    },
    "qr_hint": {
      "string_token": "qr_hint",
      "text": {
        "default": "If QR code is unavailable, manually enter 3 digit Fieldworker ID",
        "pt": "Se o código QR não estiver disponível, insira manualmente a ID do inquiridor de 3 dígitos",
        "sw": "Kama alama zako za QR hazipatikani, andika namba yako ya utambulisho yenye herufi tatu hapa"
      },
      "_row_num": 12
    },
    "submit_button": {
      "string_token": "submit_button",
      "text": {
        "default": "Submit",
        "pt": "Submeter"
      },
      "_row_num": 13
    },
    "no_result": {
      "string_token": "no_result",
      "text": {
        "default": "No Result",
        "pt": "Sem resultado"
      },
      "_row_num": 14
    },
    "hh_tab": {
      "string_token": "hh_tab",
      "text": {
        "default": "Search by Household ID",
        "pt": "Procurar por ID do Agregado"
      },
      "_row_num": 15
    },
    "hh_not_painted_tab": {
      "string_token": "hh_not_painted_tab",
      "text": "Search on Map",
      "_row_num": 16
    },
    "member_tab": {
      "string_token": "member_tab",
      "text": {
        "default": "Search by Member",
        "pt": "Procurar por Membro"
      },
      "_row_num": 17
    },
    "map_button": {
      "string_token": "map_button",
      "text": {
        "default": "Map",
        "pt": "Mapa"
      },
      "_row_num": 18
    },
    "search_button": {
      "string_token": "search_button",
      "text": {
        "default": "Search",
        "pt": "Procurar"
      },
      "_row_num": 19
    },
    "cancel_button": {
      "string_token": "cancel_button",
      "text": {
        "default": "Cancel",
        "pt": "Cancelar"
      },
      "_row_num": 20
    },
    "proceed_button": {
      "string_token": "proceed_button",
      "text": {
        "default": "Proceed",
        "pt": "Avançar"
      },
      "_row_num": 21
    },
    "fw_id_updated": {
      "string_token": "fw_id_updated",
      "text": {
        "default": "Fieldworker ID updated",
        "pt": "ID do Inquiridor actualizado"
      },
      "_row_num": 22
    },
    "yes_button": {
      "string_token": "yes_button",
      "text": "Yes",
      "_row_num": 23
    },
    "no_button": {
      "string_token": "no_button",
      "text": "No",
      "_row_num": 24
    },
    "check_painted_id": {
      "string_token": "check_painted_id",
      "text": "Does this house have a painted Bohemia ID?",
      "_row_num": 25
    },
    "roster_confirm": {
      "string_token": "roster_confirm",
      "text": "Confirm with the person in the house that this list of hh members corresponds to the hh you are in.",
      "_row_num": 26
    },
    "roster_confirm_hint": {
      "string_token": "roster_confirm_hint",
      "text": "There could have been some changes in the list of hh members, but at least one person should be correct.",
      "_row_num": 27
    },
    "hh_not_found": {
      "string_token": "hh_not_found",
      "text": "Cannot find what you're looking for?",
      "_row_num": 28
    },
    "search_hint": {
      "string_token": "search_hint",
      "text": "Try different search terms. If you still cannot find the household try to search on map or by member.",
      "_row_num": 29
    },
    "search_map_member": {
      "string_token": "search_map_member",
      "text": "Search on map or by member",
      "_row_num": 30
    },
    "member_search_hint": {
      "string_token": "member_search_hint",
      "text": "Type the name of any hh member",
      "_row_num": 31
    },
    "hh_id_search_hint": {
      "string_token": "hh_id_search_hint",
      "text": "Type the hamlet code or the household ID",
      "_row_num": 32
    },
    "hh_id_painted_hint": {
      "string_token": "hh_id_painted_hint",
      "text": "Type the household ID",
      "_row_num": 33
    }
  },
  "_locales": {
    "setting_name": "_locales",
    "_row_num": 5,
    "value": [
      {
        "display": {
          "locale": {
            "text": {
              "default": "English",
              "pt": "English",
              "sw": "English"
            }
          }
        },
        "_row_num": 7,
        "name": "default"
      },
      {
        "display": {
          "locale": {
            "text": {
              "default": "Português",
              "pt": "Português",
              "sw": "Português"
            }
          }
        },
        "_row_num": 8,
        "name": "pt"
      },
      {
        "display": {
          "locale": {
            "text": {
              "default": "Kiswahili",
              "pt": "Kiswahili",
              "sw": "Kiswahili"
            }
          }
        },
        "_row_num": 9,
        "name": "sw"
      }
    ]
  },
  "_default_locale": {
    "setting_name": "_default_locale",
    "_row_num": 5,
    "value": "default"
  }
}